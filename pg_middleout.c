 ...
 *
 * Portions of this file are derived from the PostgreSQL source, which is:
 ...
 *
 * IDENTIFICATION
 *	  contrib/pg_middleout/pg_middleout.c
 *--------------------------------------------------------------------------------
 */
...
#define	EXTENSION_NAME	"pg_middleout"
...
/*
 * State threaded through the path-tree walk.
 *
 * root				the top query's PlannerInfo.
 * calls			current estimate of how many times the SubPlan under
 *					inspection will be executed (drives the Memoize costing).
 * visited_targets	PathTargets already probed, so a target shared by several
 *					paths is not processed twice.
 */
typedef struct PathWalkerCtx
{
	PlannerInfo *root;
	double		calls;
	List	   *visited_targets;
}			PathWalkerCtx;
...
/*
 * Helper macros for path_tree_walker(): WALK recurses into one child path,
 * LIST_WALK into a list of child paths. Both assume 'walker' and 'context'
 * are in scope, following the tree_walker_callback convention.
 */
...
/*
 * path_tree_walker
 *		Recurse through a Path tree, invoking 'walker' on each child Path.
 *
 * The structural counterpart of expression_tree_walker() for paths: it knows
 * how to descend into every Path type's child path(s), while the caller's
 * 'walker' inspects the nodes. Returns true as soon as 'walker' does, following
 * the usual early-out walker convention. Errors out on an unrecognised path
 * type so a newly added node type can't be silently skipped.
 */
static bool
...
/*
 * relation_byte_size
 *		Estimate the in-memory byte size of a relation of 'tuples' rows with the
 *		given average 'width'. Copied from costsize.c; used to size Memoize
 *		cache entries.
 */
static double
...
/*
 * get_expr_width
 *		Estimate the average width, in bytes, of the value produced by the
 *		given expression, from its result type and typmod.
 */
static int32
 ...
 *		Work out the cache-key expressions and per-key hash equality operators a
 *		Memoize node needs for the given correlation parameters.
 *
 * On success returns true with the usable cache keys in *param_exprs, their
 * equality operators in *operators, and *binary_mode set. Returns false - the
 * subplan then can't be memoized - if any key contains a volatile function or
 * its type lacks a hash proc / equality operator. Simplified from the core
 * MemoizePath code; revisit if that code changes.
 */
static bool
 ...
 * because we have simplified the feature by referencing only the immediate
 * parent.
 *
 * We don't replace the SubPlan node itself. Instead, once we decide a subplan
 * is worth memoising, we swap the plan it points to (via plan_id) in the
 * global subplans/subpaths lists for a Memoize-topped plan. The SubPlan node
 * in the expression tree, and the parallel subroots list, are left untouched,
 * so this can be a read-only walker rather than a mutator.
 */
static bool
memoize_subplan_walker(Node *node, void *context)
{
	...
	if (IsA(node, SubPlan))
	{
		...
		/*
		 * Don't memoize a subplan whose tree contains volatile functions. A
		 * cache hit serves a stored result on a repeated key instead of
		 * re-executing, which would reduce the number of evaluations a volatile
		 * function is entitled to (e.g. random(), nextval(), clock_timestamp()),
		 * changing results and suppressing side effects. This mirrors the guard
		 * in core's get_memoize_path(); checking the whole subquery covers its
		 * target list, quals and any joins below it.
		 */
		...
		if (!force_memoize_subplan)
		{
			...
			elog(DEBUG5, "SubPlan Memoization costing: calls = %.0lf, current cost: %.4lf, memoized cost: %.4lf",
				 mpath->est_calls, curcost, rescan_startup_cost + ctx->calls * rescan_total_cost);
...
/*
 * subplan_hunter_path_walker
 *		Walk the chosen path tree and hand each node's SubPlan-bearing
 *		expressions to memoize_subplan_walker().
 *
 * For every node it updates ctx->calls (the execution-count estimate for any
 * SubPlan found here) and probes that node's own clause-bearing fields - the
 * pathtarget for any node, plus baserestrictinfo / join quals / agg quals
 * according to the path type - then recurses via path_tree_walker().
 */
static bool
subplan_hunter_path_walker(Node *node, void *context)
{
	...
	/*
	 * Every path carries a pathtarget whose expressions may hold a SubPlan -
	 * most notably a scalar subquery in the target list, which is attached to
	 * a ProjectionPath sitting above joins. Probe it for any path type, in one
	 * place. The same PathTarget is frequently shared (a path and its parent,
	 * or a base rel's reltarget across all of its scan paths), so remember the
	 * ones we've already handled and skip them on a second visit. We walk the
	 * tree top-down, so a shared target is processed at the upper node, whose
	 * row estimate is the right call count for a target-list subplan.
	 */
	if (pathnode->pathtarget != NULL &&
		!list_member_ptr(ctx->visited_targets, pathnode->pathtarget))
	{
		ctx->visited_targets = lappend(ctx->visited_targets,
									   pathnode->pathtarget);
		memoize_subplan_walker((Node *) pathnode->pathtarget->exprs, context);
	}

	switch (nodeTag(pathnode))
	{
		/*
		 * Base-relation scans. Their filter quals (including a correlated
		 * SubPlan in WHERE) live in the rel's baserestrictinfo, regardless of
		 * the scan method - so probe it for every scan tag, not just plain and
		 * bitmap-heap scans. For index scans this is also the correct place:
		 * create_plan builds the index-scan filter from baserestrictinfo, and
		 * the IndexOptInfo->indrestrictinfo list is shared by every candidate
		 * IndexPath on that index, so mutating it would be a cross-path side
		 * effect.
		 */
		case T_Path:
		case T_BitmapHeapPath:
		...
		case T_CustomPath:
		case T_SubqueryScanPath:
			memoize_subplan_walker((Node *) pathnode->parent->baserestrictinfo,
								   context);
			break;
		case T_NestPath:
		case T_HashPath:
...
/*
 * _PG_init
 *		Module load callback: define the extension's GUCs and install the
 *		create_upper_paths_hook that drives the memoisation pass.
 */
void
_PG_init(void)
{
