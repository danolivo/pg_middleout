...
SET pg_middleout.memoize_subplan = 'on';

-- Put back any costing issues inventing memoization tests
SET pg_middleout.force_memoize_subplan = 'on';
...
SET pg_middleout.memoize_subplan = 'off';
SELECT * FROM upper_tbl u
WHERE u.y < (SELECT avg(s.x) FROM sub_tbl s
  WHERE s.x < (SELECT avg(u2.x) FROM upper_tbl u2
    WHERE u.x <> s.x OR u2.x < u.y));
...
SET pg_middleout.force_memoize_subplan = 'off';
EXPLAIN (VERBOSE, COSTS OFF)
SELECT count(*) FROM upper_tbl u
WHERE u.y = (SELECT avg(s.x) FROM sub_tbl s WHERE s.x=u.x);
RESET enable_indexscan;
...
--
-- SubPlans the walker only reaches after broadening its coverage: one carried
-- by a path's projection target (not the base rel's reltarget), and one in the
-- filter of a scan that is neither a plain nor a bitmap-heap scan.
--
SET pg_middleout.force_memoize_subplan = 'on';

-- Target-list SubPlan computed in a scan's projection below a Sort: memoized.
EXPLAIN (VERBOSE, COSTS OFF)
SELECT u.x, (SELECT avg(s.x) FROM sub_tbl s WHERE s.x = u.y)
  FROM upper_tbl u ORDER BY 1, 2;
SELECT u.x, (SELECT avg(s.x) FROM sub_tbl s WHERE s.x = u.y)
  FROM upper_tbl u ORDER BY 1, 2;

-- Correlated SubPlan filter on a SubqueryScan (OFFSET 0 blocks pull-up): memoized.
EXPLAIN (VERBOSE, COSTS OFF)
SELECT * FROM (SELECT x, y FROM upper_tbl OFFSET 0) ss
WHERE ss.y < (SELECT avg(s.x) FROM sub_tbl s WHERE s.x = ss.x) ORDER BY 1, 2;
SELECT * FROM (SELECT x, y FROM upper_tbl OFFSET 0) ss
WHERE ss.y < (SELECT avg(s.x) FROM sub_tbl s WHERE s.x = ss.x) ORDER BY 1, 2;

-- A volatile function in the subplan tree blocks memoization: a cache hit would
-- skip evaluations the volatile function is entitled to. Plan keeps a plain
-- SubPlan (no Memoize); random() < 1.0 is always true, so the result is stable.
EXPLAIN (VERBOSE, COSTS OFF)
SELECT count(*) FROM upper_tbl u
WHERE u.y < (SELECT avg(s.x) FROM sub_tbl s WHERE s.x = u.x AND random() < 1.0);
SELECT count(*) FROM upper_tbl u
WHERE u.y < (SELECT avg(s.x) FROM sub_tbl s WHERE s.x = u.x AND random() < 1.0);
...
RESET pg_middleout.memoize_subplan;
RESET pg_middleout.force_memoize_subplan;
